...
whenever $\varg (a) = (x, y)$,
\\
\item $\varg: 2^\args \to 2^{\nat \times \nat}$ be a function s.t. $\varg(A^{'}) = \{\varg(a)$ $|$ $a \in \mathcal{A^{'}}\}$,
...
\\
\item $\vatt: 2^\att \to 2^{\nat \times \nat}$ be a function s.t. $\vatt(R^{'}) = \{\vatt((a, b))$ $|$ $(a, b) \in R^{'}\}$,
...
\item $max: 2^{\nat} \to \nat$ be a function s.t. it returns the maximum value amongst the non-empty set of natural numbers given as the input.

\item $\omega:  2^{\nat \times \nat} \to 2^{\nat}$ be a function s.t. $\omega(\mathcal{X}) = \{v^t(a, b)$ $|$ $(a, b) \in \mathcal{X}\}$,
...
[c]{lll}
0 &  & v_{max}=0\\
\frac{v^{+}}{v^{+}+v^{-}+\frac{1}{max(\omega(\mathcal{X}))}} &  & \text{otherwise}%
\end{array}
\right.
\]
